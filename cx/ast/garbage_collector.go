...
func doDisplaceReferences(prgrm *CXProgram, updated *map[types.Pointer]types.Pointer, atOffset types.Pointer, plusOff types.Pointer, baseType int, declSpecs []int) {
	...
	if declSpecs[0] == constants.DECL_SLICE {
		if (numDeclSpecs > 1 &&
			...
			for c := types.Pointer(0); c < sliceLen; c++ {
				...
				updateDisplaceReference(prgrm, updated, heapOffset+offsetToElements+c*types.TYPE_POINTER_SIZE, plusOff)
			}
		}
	}
}
...
func MarkObjectsTree(prgrm *CXProgram, offset types.Pointer, baseType int, declSpecs []int) {
	...
	if offset > lenMem || offset+types.TYPE_POINTER_SIZE > lenMem {
		return
	}
	...
	if declSpecs[0] == constants.DECL_SLICE {
		if (numDeclSpecs > 1 &&
			...
			for c := types.Pointer(0); c < sliceLen; c++ {
				...
				MarkObjectsTree(prgrm, heapOffset+offsetToElements+c*types.TYPE_POINTER_SIZE, baseType, declSpecs[1:])
			}
		}
	}
}
...
func updatePointerTree(prgrm *CXProgram, atOffset types.Pointer, oldAddr, newAddr types.Pointer, baseType int, declSpecs []int) {
	var numDeclSpecs = len(declSpecs)
		...
		if (numDeclSpecs > 1 &&
			...
			for c := types.Pointer(0); c < sliceLen; c++ {
				...
				if cHeapOffset == oldAddr {
					updatePointerTree(prgrm, heapOffset+offsetToElements+c*types.TYPE_POINTER_SIZE, oldAddr, newAddr, baseType, declSpecs[1:])// TODO: PTR remove hardcode
				}
			}
		}
	}
	...
	for _, pkg := range prgrm.Packages {
		for _, glbl := range pkg.Globals {
			if (glbl.IsPointer || glbl.IsSlice || glbl.Type == constants.TYPE_STR) && glbl.CustomType == nil {
				...
				updatePointerTree(prgrm, glbl.Offset, oldAddr, newAddr, glbl.Type, glbl.DeclarationSpecifiers[1:]) // TODO: PTR remove hardcode
			}
